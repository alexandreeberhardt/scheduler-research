...
noyau réel}. Le critère principal est l'amélioration du temps de réponse. Le
...
boucle fermée a déjà montré un gain de l'ordre de \mbox{$-10{,}5\,\%$} sur le
temps de réponse moyen face à CFS dans un émulateur.

\medskip
Les objectifs spécifiques sont :
\begin{itemize}[leftmargin=1.5em,itemsep=3pt]
  \item Renforcer la formulation \emph{candidate-based} en remplaçant le
    \emph{teacher} heuristique par un \textbf{teacher quasi-optimal à lookahead}
    (étiquetage obtenu en simulant plusieurs pas à l'avance dans l'émulateur), puis distillation
    vers un modèle causal.
  \item Réduire le \emph{covariate shift} entre états d'entraînement et états
    auto-induits par un \textbf{réentraînement itératif de type DAgger}.
  ...
  \item \textbf{Déployer} la politique distillée dans un noyau réel via
    \texttt{sched\_ext} (ordonnanceur en espace utilisateur de type
    \texttt{scx\_rustland}) et la comparer à EEVDF en boucle fermée sur des
    charges réelles.
\end{itemize}
...
littérature : les rares travaux existants (\textit{KernelOracle}~[3],
[1], [2], [4]) se limitent à la prédiction hors ligne, sans déploiement ni
évaluation en boucle fermée dans un noyau réel.

\medskip
Le projet s'appuie sur une fondation existante (GLO-7030) comportant deux volets :
(i) un pipeline d'imitation séquentielle de type LSTM sur traces
...
candidats à partir d'un état d'ordonnancement riche, évaluée en boucle fermée
dans un émulateur. C'est ce second volet qui constitue la contribution
principale.

\medskip
Le projet corrige les limites de ces travaux préliminaires : un
